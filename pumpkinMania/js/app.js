...
start_btn.addEventListener('click', () => {
	...
	let holee = [false, false, false, false, false]
	let points = 0

	const game = setInterval(() => {
		console.log("game is running")
		// Here we are taking a random hole
		// from where pumpkin comes out
		let ran = Math.floor(Math.random() * 5)
		holee[ran] = true

		// This part is used for taking the
		// pumpkin out of the desired hole
		let set_img = document.createElement('img')
		set_img.setAttribute('src',
'../img/halloween-1298060_960_720.png')
		set_img.setAttribute('class', 'pumpkin')
		holee.appendChild(set_img)

		// This part is used for putting
		// the pumpkin back in the hole
		setTimeout(() => {
